 ...
 */
export const triggerScheduleImmediate = functions.https.onRequest(async (req, res) => {
 ...
 */
export const debugUserSchedule = functions.https.onRequest(async (req, res) => {
    ...
    try {
        ...
        if (!fcmToken) log(`Token: ${(userData as any).fcmToken ? 'Using userData' : 'Missing'}`);
        ...
        for (const code of enrolledIds) {
            ...
            if (doc.exists) {
                courseDetails.push({ id: doc.id, ...doc.data() });
                log(`Found Course: ${code} - Day: ${doc.data()?.day} - Sessions: ${JSON.stringify(doc.data()?.sessions)}`);
            } else {
        ...
        // ... (Simplified check)
        const matchesDay = (dayStr: string, targetDay: string): boolean => {
            if (!dayStr) return false;
            const days = dayStr.toUpperCase().split(/\s+/);
            for (const d of days) {
                if (d.includes(targetDay)) return true;
            }
            return false;
        };
        ...
        const todaysClasses = courseDetails.filter(c => {
            if (c.sessions && Array.isArray(c.sessions)) {
                return c.sessions.some((s: any) => matchesDay(s.day, dayLetter));
            }
...
async function processUserSchedule(
    ...
    for (const chunk of chunks) {
        const q = await db.collection(`courses_${semesterCode}`)
            ...
            .get();
        q.forEach((d: any) => courseDetails.push({ id: d.id, ...d.data() }));
    }
    ...
    const exceptions = scheduleDoc.exists ? (scheduleDoc.data()?.exceptions || []) : [];

    // Helper to check if class is cancelled
    const dateString = targetDate.toISOString().split('T')[0];
    const isCancelled = (code: string) => {
        return exceptions.some((e: any) =>
            e.date === dateString &&
            e.courseCode === code &&
            e.type === 'cancel'
        );
    ...
    const makeups = exceptions.filter((e: any) =>
        e.date === dateString &&
        e.type === 'makeup'
    ).map((e: any) => ({
        code: e.courseCode,
        startTime: e.startTime, // Assuming "HH:mm AM/PM" format from repo
        room: e.room || "TBA",
        original: {}, // Dummy
        isMakeup: true
    }));
    ...
    let todaysClasses = courseDetails.filter(c => {
        ...
        if (isCancelled(c.code || c.courseCode)) return false;
        ...
        let session;
        if (c.sessions) {
            session = c.sessions.find((s: any) => matchesDay(s.day, dayLetter));
        } else {
            session = { startTime: c.startTime, room: c.room };
        }
        return {
            code: c.code || c.courseCode,
            startTime: session?.startTime || "TBA",
            room: session?.room || c.room || "TBA", // Fallback chain for room
            original: c,
            isMakeup: false
        };
    });
        ...
        await admin.messaging().send({
            ...
            android: {
                ...
                notification: {
                    ...
                    priority: 'high',
                    defaultSound: false, // Disable default sound
                    defaultVibrateTimings: false, // Disable default vibration pattern
                    vibrateTimingsMillis: [0, 800, 400, 800, 400, 800, 400, 800, 400, 800], // ~5+ seconds vibration
                    visibility: 'public',
                }
            }
        });
 ...
 */
export const onAdvisingSlotUpdated = functions.firestore
    ...
    .onUpdate(async (change, context) => {
        ...
        if (!afterSlot || !afterSlot.startTime) return;
        if (beforeSlot && beforeSlot.startTime === afterSlot.startTime) return;
 ...
 */
async function scheduleAdvisingTask(
    token: string,
    advisingTime: Date,
    minutesBefore: number,
    displayTime: string
...
export const onBroadcastCreated = functions.firestore
    ...
    .onCreate(async (snap, context) => {
        ...
        if (data.status === 'sent' || data.status === 'scheduled') return;
 ...
 */
async function scheduleBroadcastTask(
    broadcastId: string,
    title: string,
    body: string,
    ...
    const payload = {
        topic: 'all_users', // Special handler for topic in worker
        title,
        body,
        data: {
 ...
 */
async function sendBroadcastImmediate(title: string, body: string, link?: string) {
...
/**
 * 7. Automated Update Trigger
 * Listens to config/app_info and sends broadcast when latestVersion changes.
 */
export const onAppInfoUpdated = functions.firestore
    .document('config/app_info')
    .onUpdate(async (change, _context) => {
        ...
        // Only notify if version changed AND we have a new version string
        if (newVersion && newVersion !== oldVersion) {
            console.log(`Version changed from ${oldVersion} to ${newVersion}. Sending broadcast.`);

            const message: admin.messaging.Message = {
                notification: {
                    title: "Update Available! 🚀",
                    body: `Version ${newVersion} is now available. Tap to update.`,
                },
                data: {
                    click_action: "FLUTTER_NOTIFICATION_CLICK",
                    link: downloadUrl || ""
