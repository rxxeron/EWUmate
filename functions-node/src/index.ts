 ...
 */
async function _getAdminSecret(): Promise<string> {
    try {
...
async function scheduleForTask(userId: string, taskId: string, fcmToken: string, task: any) {
...
async function processUserForNotifications(userId: string, userData: any, targetDay: string, dateString: string) {
    ...
    const makeups = exceptions.filter((ex: any) => ex.type === 'makeup');
    ...
    for (const cls of parsedClasses) {
        ...
        for (const offset of offsets) {
            ...
            const safeCode = (cls.courseCode as string).replace(/\s+/g, '');
...
async function scheduleNotificationTask(taskId: string, userId: string, fcmToken: string, title: string, body: string, time: Date) {
