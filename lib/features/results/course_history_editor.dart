...
class _CourseHistoryEditorState extends State<CourseHistoryEditor> {
  ...
  Future<void> _importCurrentSemester() async {
    setState(() => _loading = true);
    try {
      final currentResults = await _repo.fetchCurrentEnrolledResults();
      if (currentResults.isEmpty) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("No enrolled courses found for current semester.")),
          );
        }
      ...
      // Logic to determine semester name (usually the first result's metadata or just fetch from active_semester)
      final academicRepo = AcademicRepository();
      final semesterName = await academicRepo.getCurrentSemesterCode();

      setState(() {
        if (!_history.containsKey(semesterName)) {
          _history[semesterName] = <String, dynamic>{};
        }
        for (var res in currentResults) {
          // Only add if not already there to avoid overwriting existing grades
          if (!_history[semesterName].containsKey(res.courseCode)) {
            _history[semesterName][res.courseCode] = "Ongoing";
          }
        }
        _loading = false;
      });
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("Imported ${currentResults.length} courses for $semesterName!")),
        );
      }
    } catch (e) {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Edit Course History", style: TextStyle(color: Colors.white)),
        ...
        actions: [
          IconButton(
            ...
            onPressed: _loading ? null : _importCurrentSemester,
            tooltip: "Import Current Semester",
          ),
          IconButton(
