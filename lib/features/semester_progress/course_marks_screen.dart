...
class _CourseMarksScreenState extends State<CourseMarksScreen>
  ...
  Future<void> _deleteQuiz(int index, bool isShort) async {
    final success = isShort
        ? await _repo.deleteShortQuizMark(
            widget.semesterCode, widget.courseCode, index)
        : await _repo.deleteQuizMark(
            widget.semesterCode, widget.courseCode, index);
 
    if (success) {
      _showSnack("Mark deleted", isError: false);
      _loadData();
    }
  }
 
  Future<void> _editQuizDialog(int index, double currentVal, bool isShort) async {
    final ctrl = TextEditingController(text: currentVal.toString());
    final newVal = await showDialog<double>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: Text("Edit ${isShort ? 'Short Quiz' : 'Quiz'}"),
        content: TextField(
          controller: ctrl,
    ...
    if (newVal != null) {
      final success = isShort
          ? await _repo.updateShortQuizMark(
              widget.semesterCode, widget.courseCode, index, newVal)
          : await _repo.updateQuizMark(
              widget.semesterCode, widget.courseCode, index, newVal);
 
      if (success) {
        _showSnack("Mark updated", isError: false);
        _loadData();
      }
    }
  }
        ...
        Wrap(
          ...
          children: marks.asMap().entries.map((e) {
            return GestureDetector(
              onTap: () => _editQuizDialog(e.key, e.value, isShort),
              child: Container(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    IconButton(
                      ...
                      onPressed: () => _deleteQuiz(e.key, isShort),
                      padding: EdgeInsets.zero,
                      constraints: const BoxConstraints(),
                    ),
                  ],
